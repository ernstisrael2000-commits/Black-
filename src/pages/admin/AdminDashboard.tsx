...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: 'Commandes totales', value: stats?.totalOrders || 0, icon: <ShoppingBag size={20} />, color: 'text-blue-400 bg-blue-400/10' },
    { label: 'Revenu total (HTG)', value: formatPrice(stats?.totalRevenue || 0, 'HTG'), icon: <TrendingUp size={20} />, color: 'text-[#C9A84C] bg-[#C9A84C]/10' },
    { label: 'En attente', value: stats?.pendingOrders || 0, icon: <Package size={20} />, color: 'text-yellow-400 bg-yellow-400/10' },
    { label: 'Commandes livrées', value: stats?.completedOrders || 0, icon: <Users size={20} />, color: 'text-green-400 bg-green-400/10' },
  ];

  return (
    <div className="p-6 space-y-6">
      <div>
        <h2 className="font-display text-2xl font-bold text-white mb-1">Tableau de bord</h2>
      ...
      <div className="bg-[#111] border border-[#1F1F1F] rounded-2xl overflow-hidden">
        ...
        {!stats?.recentOrders?.length ? (
          <div className="p-8 text-center text-gray-500 text-sm">Aucune commande pour l'instant</div>
        ) : (
            ...
            {stats.recentOrders.map((order: any) => (
              <div key={order.id} className="flex items-center justify-between p-4 hover:bg-[#1A1A1A] transition-colors">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{order.userEmail} — {formatDate(order.createdAt)}</p>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <Link to={`/admin/commandes/${order.id}`} className="text-gray-500 hover:text-white">
                    <Eye size={16} />
                  </Link>
                </div>
              </div>
      ...
      {lowStock.length > 0 && (
        <div className="bg-[#111] border border-yellow-500/20 rounded-2xl overflow-hidden">
          <div className="flex items-center gap-2 p-5 border-b border-[#1F1F1F]">
            <AlertTriangle size={16} className="text-yellow-400" />
            <h3 className="font-semibold text-white">Stock faible ({lowStock.length})</h3>
          </div>
          <div className="divide-y divide-[#1F1F1F]">
            {lowStock.slice(0, 5).map((product) => (
              <div key={product.id} className="flex items-center justify-between p-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl overflow-hidden bg-[#141414]">
